...
def main():
    ...
    # 读取数据
    train_data_path = cfg.ner_data_dir+r'\train.txt'
    eval_data_path = cfg.ner_data_dir+r'\dev.txt'
    ...
    for epoch in trange(int(EPOCH), desc="Epoch"):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            batch_acc = accuracy(train_logits, label_ids)
            train_accuracy += batch_acc
            train_loss += loss.item()
            train_examples += input_ids.size(0)
            train_steps += 1
            if step % 1 == 0:  # 每训练10个batch输出一次loss
                logger.info('Batch %d , batch_loss = %.4f, batch_accuracy = %.4f' % (global_step + 1 , loss , batch_acc))
            loss.backward()
            optimizer.step()
            global_step += 1

            if args.do_eval and step % 1 == 0:  # 每训练100个batch进行一次eval,计算一次accuracy

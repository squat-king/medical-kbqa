...
from data_preparing.max_cut import CutWords
...
class MedicalGraph:
    ...
    def collect_medical(self):
        ...
        for item in self.col.find():
            ...
            inspects += inspect
            attributes = basic_info['attributes'] # list
            # 成因及预防
            ...
            for attr in attributes:
                ...
                if len(attr_pair) == 2:
                    key = attr_pair[0]
                    value = attr_pair[1]
                    data[key] = value # 就诊科室、治疗方式、治疗周期、治愈率等
            # 检查
            inspects = item['inspect_info']
            checks = []
            for inspect in inspects:
                check_name = self.get_inspect(inspect)
                if check_name:
            ...
            for attr, value in data.items():
                # 将属性翻译为英文
                att_name_eng = self.key_dict.get(attr)
                # 重新添加到data_modify中，保存的是英文的键值对
                if att_name_eng:
                    data_modify[att_name_eng] = value
                # 过滤空格
                if att_name_eng in ['yibao_status', 'get_prob', 'easy_get', 'get_way', "cure_lasttime", "cured_prob"]:
                    data_modify[att_name_eng] = value.replace(' ','').replace('\t','')
                # 将's1 s2 s3 ...'格式的字符串转换为list
                elif att_name_eng in ['cure_department', 'cure_way', 'common_drug']:
                    data_modify[att_name_eng] = [i for i in value.split(' ') if i]
                # 分词，使用双向最大向前匹配法
                elif att_name_eng in ['acompany']:
                    acompany = [i for i in self.cuter.max_biward_cut(data_modify[att_name_eng]) if len(i) > 1]
                    data_modify[att_name_eng] = acompany

            try:
                self.db['medical_eng'].insert(data_modify)
                count += 1
                print(count)
    ...
    def collection_export(self,col_name,filepath):
        ...
        with open(filepath, mode='w', encoding='utf-8') as f:
            for i in col.find():
                # print(type(i))
                s=json_util.dumps(i,ensure_ascii=False)
                f.write(s+"\n")
        f.close()
if __name__ == '__main__':
    ...
    # 从mongodb中导出成json格式
    handler.collection_export('medical_eng',config.medical_data)